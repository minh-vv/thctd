...
Type* compileLValue(void) {
  ...
  case OBJ_FUNCTION:
    ...
    // Not fully supported in strict KPL/Pascal by variable address without extra context.
    genVariableAddress(var); // Attempt to treat as variable
...
void compileCallSt(void) {
  ...
  if (isPredefinedProcedure(proc)) {
    ...
    while(scope != proc->procAttrs->scope->outer) { level++; scope = scope->outer; }
...
void compileWhileSt(void) {
  CodeAddress beginLoop = getCurrentCodeAddress();
  eat(KW_WHILE);
  compileCondition();
  Instruction* fj = genFJ(DC_VALUE);
  eat(KW_DO);
...
void compileForSt(void) {
  ...
  // Parse the loop variable manually to keep track of the object
  eat(TK_IDENT);
  var = checkDeclaredLValueIdent(currentToken->string);
  
  // Generate address for initialization
  if (var->kind == OBJ_VARIABLE) {
      genVariableAddress(var);
      varType = var->varAttrs->type;
  } else if (var->kind == OBJ_PARAMETER) {
      // Loop variable usually must be local variable in Pascal/KPL? 
      // Assuming parameter is okay if it's l-value.
      genVariableAddress(var);
      varType = var->paramAttrs->type;
  } else {
      error(ERR_INVALID_LVALUE, currentToken->lineNo, currentToken->colNo);
      varType = NULL; // unreachable
  }
  
  // Check that it is of basic type (int/char) usually? 
  ...
  genST(); // Init assignment: v := init_val
  ...
  case TK_IDENT:
    ...
    case OBJ_VARIABLE:
      if (obj->varAttrs->type->typeClass == TP_ARRAY) {
        ...
        genLI(); // Load value from element address
      } else {
    ...
    case OBJ_FUNCTION:
      if (isPredefinedFunction(obj)) {
        ...
        int level = 0;
        Scope* scope = symtab->currentScope;
        while(scope != obj->funcAttrs->scope->outer) { level++; scope = scope->outer; }
